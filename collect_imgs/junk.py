...
def train_rl():
    ...
    for ep in range(1000):
            ...
            while (not done) and (not env.episode_over) and (not success):  
                ...
                geo_dist_traveled_by_agent = env.sim.geodesic_distance(init_agent_position, agent_position)
                SPL = float(success)*init_geodesic_dist/max(dist_traveled_by_agent, init_geodesic_dist)
                dist_to_target = env.sim.geodesic_distance(agent_position, target_position)

                print('act: ', allowed_actions[act])
                print('agent pos: ', agent_position)
                print('target pos: ', target_position)
                print('total dist traveled by agent: ', geo_dist_traveled_by_agent) 
                print('old pointgoal: ', s["pointgoal"])
                print('old heading: ', s["heading"])
                print('new heading: ', s2["heading"])
                print('pointgoal: ', s2["pointgoal"])
                print('pointgoal diff: ', s2["pointgoal"]-s["pointgoal"]) 
                print('dist to target: ', dist_to_target) 
                print('-'*25)


                pg_diff = s2["pointgoal"]-s["pointgoal"]
                if (np.abs(pg_diff[0]) >= 1.0 or np.abs(pg_diff[2]) >= 1.0):
                     print('pg diff!!! ', pg_diff)
                     sys.exit()
